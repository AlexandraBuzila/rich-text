...
public class HTMLDiffer implements Differ {
	...
	public void diff(TextNodeComparator ancestorComparator, TextNodeComparator leftComparator,
		...
		org.eclipse.compare.rangedifferencer.RangeDifference[] differences = RangeDifferencer.findDifferences(
		...
		for (RangeDifference d : pdifferences) {
			...
			if (tempKind == RangeDifference.ANCESTOR) {
				// ignore, we won't show pseudo-conflicts currently (left and
				// right have the same change)
				continue;
			}

			if (d.leftStart() > currentIndexLeft) {
				ancestorComparator.handlePossibleChangedPart(currentIndexLeft, d.leftStart(), currentIndexAncestor,
						d.ancestorStart(), leftComparator);
			}
			...
			if (tempKind == RangeDifference.CONFLICT || tempKind == RangeDifference.LEFT) {
				...
				if (d.leftLength() > 0) {
					ancestorComparator.markAsDeleted(d.leftStart(), d.leftEnd(), leftComparator, d.ancestorStart(),
			...
			if (tempKind == RangeDifference.CONFLICT || tempKind == RangeDifference.RIGHT) {
				...
				if (d.rightLength() > 0) {
					ancestorComparator.markAsDeleted(d.rightStart(), d.rightEnd(), rightComparator, d.ancestorStart(),
							ModificationType.ADDED);
				}
			}
		...
		if (currentIndexLeft < leftComparator.getRangeCount()) {
			ancestorComparator.handlePossibleChangedPart(currentIndexLeft, leftComparator.getRangeCount(),
					currentIndexAncestor, ancestorComparator.getRangeCount(), leftComparator);
		}
		if (currentIndexRight < rightComparator.getRangeCount()) {
			ancestorComparator.handlePossibleChangedPart(currentIndexRight, rightComparator.getRangeCount(),
					currentIndexAncestor, ancestorComparator.getRangeCount(), rightComparator);
		}
	...
	public void diff(TextNodeComparator leftComparator, TextNodeComparator rightComparator) throws SAXException {
		...
		for (RangeDifference d : pdifferences) {
			...
			if (d.leftStart() > currentIndexLeft) {
				rightComparator.handlePossibleChangedPart(currentIndexLeft, d.leftStart(), currentIndexRight,
						d.rightStart(), leftComparator);
			}
			if (d.leftLength() > 0) {
	...
	private List<RangeDifference> preProcess(RangeDifference[] differences) {
		...
		for (int i = 0; i < differences.length; i++) {
			...
			while (i + 1 < differences.length
					...
					&& score(leftLength, differences[i + 1].leftLength(), rightLength, differences[i + 1].rightLength()) > (differences[i + 1]
							.leftStart() - leftEnd)) {
			...
			newRanges.add(RangeDifferenceHelper.getRangeDifference(kind, rightStart, rightLength, leftStart,
					leftLength, ancestorStart, ancestorLength));
		}
